...
{
	m_fMinRange1 = GetWpnData().primary.minRange;
	m_fMaxRange1 = GetWpnData().primary.maxRange;
	m_fMinRange2 = GetWpnData().secondary.minRange;
	m_fMaxRange2 = GetWpnData().secondary.maxRange;

	m_bFiresUnderwater = GetWpnData().primary.canFireUnderwater;
	m_bAltFiresUnderwater = GetWpnData().secondary.canFireUnderwater;
...
void CWeaponCustom::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, Vector &vecShootOrigin, Vector &vecShootDir )
{
	...
	pOperator->FireBullets( 1, vecShootOrigin, vecShootDir, VECTOR_CONE_PRECALCULATED,
		...
		info.m_iShots = GetWpnData().primary.shotCount;
		info.m_flDamage = GetWpnData().primary.damage;
		info.m_flPlayerDamage = GetWpnData().primary.damage;
...
void CWeaponCustom::ShootBullets( bool isPrimary, bool usePrimaryAmmo )
{
	...
	// Make sure we don't fire more than the amount in the clip, if this weapon uses clips
		if (!GetWpnData().primary.infiniteAmmoEnabled)
		{
			if (UsesClipsForAmmo1())
			{
	...
	else if (usePrimaryAmmo)
	{
		m_iSecondaryAttacks++;
		// Make sure we don't fire more than the amount in the clip, if this weapon uses clips
		if (!GetWpnData().secondary.infiniteAmmoEnabled)
		{
			if (UsesClipsForAmmo1())
			{
				if (iBulletsToFire > m_iClip1)
	...
	{
		info.m_iShots = GetWpnData().secondary.shotCount;
		info.m_flDamage = GetWpnData().secondary.damage;
		info.m_flPlayerDamage = GetWpnData().secondary.damage;
	...
	if (isPrimary)
	{
		AddViewKick(GetWpnData().primary.recoilEasyDampen, GetWpnData().primary.recoilDegrees, GetWpnData().primary.recoilSeconds);
	}
	else
	{
		AddViewKick(GetWpnData().secondary.recoilEasyDampen, GetWpnData().secondary.recoilDegrees, GetWpnData().secondary.recoilSeconds);
	}
...
void CWeaponCustom::ShootBulletsRight(bool isPrimary, bool usePrimaryAmmo)
{
	...
	if (isPrimary)
	{
		AddViewKick(GetWpnData().primary.recoilEasyDampen, GetWpnData().primary.recoilDegrees, GetWpnData().secondary.recoilSeconds);
	}
	else
	{
		AddViewKick(GetWpnData().secondary.recoilEasyDampen, GetWpnData().secondary.recoilDegrees, GetWpnData().secondary.recoilSeconds);
...
//	m_hMissile->m_hOwner = this;
	...
	if (isPrimary)
	{
		if (GetWpnData().primary.hasRecoilRPGMissle)
		{
			AddViewKick(GetWpnData().primary.recoilEasyDampen, GetWpnData().primary.recoilDegrees, GetWpnData().primary.recoilSeconds);
		}
	}
	...
	{
		if (GetWpnData().secondary.hasRecoilRPGMissle)
		{
			AddViewKick(GetWpnData().secondary.recoilEasyDampen, GetWpnData().secondary.recoilDegrees, GetWpnData().secondary.recoilSeconds);
		}
	}
...
void CWeaponCustom::ShootSMGGrenade(bool isPrimary, bool usePrimaryAmmo)
{
	...
	if (isPrimary)
	{
		pGrenade->SetDamage(GetWpnData().primary.SMGGrenadeDamage);
	}
	else
	{
		pGrenade->SetDamage(GetWpnData().secondary.SMGGrenadeDamage);
	}
	...
	if (isPrimary)
	{
		if (GetWpnData().primary.hasRecoilSMGGrenade)
		{
			AddViewKick(GetWpnData().primary.recoilEasyDampen, GetWpnData().primary.recoilDegrees, GetWpnData().primary.recoilSeconds);
		}
	}
	...
	{
		if (GetWpnData().secondary.hasRecoilSMGGrenade)
		{
			AddViewKick(GetWpnData().secondary.recoilEasyDampen, GetWpnData().secondary.recoilDegrees, GetWpnData().secondary.recoilSeconds);
		}
	}
...
void CWeaponCustom::ShootAR2EnergyBall(bool isPrimary, bool usePrimaryAmmo)
{
	...
	if (isPrimary)
	{
		CreateCombineBall(vecSrc,
			vecVelocity,
			GetWpnData().primary.combineBallRadius,
			GetWpnData().primary.combineBallMass,
			GetWpnData().primary.combineBallDuration,
			pOwner);
	}
	else
	{
		CreateCombineBall(vecSrc,
			vecVelocity,
			GetWpnData().secondary.combineBallRadius,
			GetWpnData().secondary.combineBallMass,
			GetWpnData().secondary.combineBallDuration,
			pOwner);
	}
...
{
	if (GetWpnData().primary.hasFire)
	{
		CBasePlayer *pPlayer = ToBasePlayer(GetOwner());

		if (GetWpnData().primary.missleEnabled)
		{
			if (GetWpnData().m_sDualWeapons)
			{
				if (GetWpnData().primary.fireBoth)
				{
		...
		if (GetWpnData().primary.SMGGrenadeEnabled)
		{
			if (GetWpnData().m_sDualWeapons)
			{
				if (GetWpnData().primary.fireBoth)
		...
		if (GetWpnData().primary.AR2EnergyBallEnabled)
		{
			if (GetWpnData().m_sDualWeapons)
			{
				if (GetWpnData().primary.fireBoth)
...
bool CWeaponCustom::Reload( void )
{
	if (GetWpnData().m_sUsesZoom)
	{
		...
		color32 zoomColor = { GetWpnData().m_sZoomColorRed, GetWpnData().m_sZoomColorGreen, GetWpnData().m_sZoomColorBlue, GetWpnData().m_sZoomColorAlpha };
...
{
	if (GetWpnData().secondary.hasFire)
	{
		CBasePlayer *pPlayer = ToBasePlayer(GetOwner());

		if (GetWpnData().secondary.missleEnabled)
		{
			if (GetWpnData().m_sDualWeapons)
			{
				if (GetWpnData().secondary.fireBoth)
				{
					ShootProjectileRight(false, GetWpnData().m_sUsePrimaryAmmo);
					ShootProjectileLeft(false, GetWpnData().m_sUsePrimaryAmmo);
				}
			}
			else
			{
				ShootProjectile(false, GetWpnData().m_sUsePrimaryAmmo);
			}
		}
		if (GetWpnData().secondary.SMGGrenadeEnabled)
		{
			if (GetWpnData().m_sDualWeapons)
			{
				if (GetWpnData().secondary.fireBoth)
				{
					ShootSMGGrenadeRight(false, GetWpnData().m_sUsePrimaryAmmo);
					ShootSMGGrenadeLeft(false, GetWpnData().m_sUsePrimaryAmmo);
				}
			}
			else
			{
				ShootSMGGrenade(false, GetWpnData().m_sUsePrimaryAmmo);
			}
		}
		if (GetWpnData().secondary.AR2EnergyBallEnabled)
		{
			if (GetWpnData().m_sDualWeapons)
			{
				if (GetWpnData().secondary.fireBoth)
				{
					ShootAR2EnergyBallRight(false, GetWpnData().m_sUsePrimaryAmmo);
					ShootAR2EnergyBallLeft(false, GetWpnData().m_sUsePrimaryAmmo);
				}
			}
			else
			{
				ShootAR2EnergyBall(false, GetWpnData().m_sUsePrimaryAmmo);
			}
		}
