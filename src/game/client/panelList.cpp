...
//	Constructor
////////////////////////////////////////////////////////////////////////////////
PanelList::PanelList(Panel *parent, const char *name) : Panel(parent, name)
{
	...
	m_pSelImage = new BasePanel( this, "SelectImage");
...
int PanelList::AddPanel( Panel* pPanel, bool isSelectable)
{
	return AddPanel(pPanel, NULL, isSelectable);
}


int PanelList::AddPanel( Panel* pPanel, char* header,  bool isSelectable)
{
	int headId = -1;
	...
	if (header!=NULL)
	{
		...
		{
			vgui::Panel* pPanel=new HeaderPanel(this, header);
			pPanel->SetZPos(25);
...
void PanelList::ResetAllChildPos(  )
{
	m_iSelected = 0;
...
void PanelList::SetHeight( int amount )
{
	if (this->IsProportional())
...
void PanelList::Reset()
{

}


void PanelList::OnSizeChanged(int newWide, int newTall)
{
...
void PanelList::OnThink( void )
{
	Update();
	BaseClass::OnThink();
}

void PanelList::OnCommand(char* command)
{
...
void PanelList::OnRequestUpdate(const char* PanelName)
{
	ResetAllChildPos();
	//OnItemClicked( PanelName );
}
void PanelList::Update( void )
{
...
void PanelList::OnSliderMoved(int value)
{
...
void PanelList::MoveBy( int amount, bool slider )
{
	int x,y;
	for (int i=0; i<m_pPanelList.Count(); i++)
	{
...
void PanelList::Paint()
{
	BaseClass::Paint();
	PaintBorder();
}

void PanelList::PaintBackground()
{
	if (!m_bPaintBg)
		return;
...
void PanelList::updateSize( void )
{
	OnSizeChanged(GetWide(), GetTall());
}

int PanelList::FindItemUsingIdent(const char *pszIdent)
{
	for(int i = 0; i < m_pPanelList.Count();i++)
	{
		vgui::Panel *pItem = m_pPanelList[i]->pPanel;
...
int PanelList::FindHeaderUsingIdent(const char *pszIdent)
{
	for(int i = 0; i < m_pHeaderList.Count();i++)
	{
...
void PanelList::OnKeyCodeTyped(KeyCode code)
{
	//Msg("KeyCode %d\n", code);
...
void PanelList::OnCursorMoved( int x, int y)
{
}


void PanelList::OnMouseWheeled(int delta)
{
	delta*=-1;
	if (m_pPanelList.Count() > 0)
	{
...
void PanelList::OnSetSelected(const char* name)
{
		OnItemClicked(name);
}

void PanelList::OnItemClicked(const char* name)
{
...
void PanelList::OnItemDoubleClicked(const char* name)
{
...
//void PanelList::OnMouseDoublePressed(MouseCode code)
//{
//	Msg("Mouse clicked\n");
//}
...
HeaderPanel::HeaderPanel(Panel *parent, const char *name) : BasePanel(parent, name)
{
...
void HeaderPanel::PaintBackground()
{
	return;
...
void HeaderPanel::OnSizeChanged(int newWide, int newTall)
{
...
SubPanel::SubPanel(Panel *parent, const char *name, const char* mdlName, const char* title, int slot) : BasePanel(parent, name)
{

	m_pNameLab = new vgui::Label(this, "Label", title);
	model = new CAdvModelPanel(this, "Model");
...
void SubPanel::OnSizeChanged(int newWide, int newTall)
{
	...
	model->SetBounds(newTall + MARG, y2, w, h);
}